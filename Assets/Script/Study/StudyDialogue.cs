...
public class StudyDialogue : MonoBehaviour
{
    // 提示区
    ...
    private void TipAreaInit()
    {
        ...
        {
            //Debug.Log("背景图："+ _curDialogueList[0].backgroundIcon);
    ...
    private void PlayDialogueVoice()
    {
        ...
        if (_curDialogueIndex < _dialogueNum)
        {
            ...
            {
                //Debug.Log("背景图1：" + _curDialogueList[_curDialogueIndex].backgroundIcon);
    ...
    {
        //float time = AudicoManager.instance.Play("study", StringConst.SentencePath + _assetsPath + NeedSpeakList[_curSpeakIndex].cluesVoice2[0].Replace(".mp3", "").Replace(" ", ""));
        //_gameUIInstance.SetVoiceEffect(go, time);
    }
    ...
    private void OnSpeak()
    {
        ...
        {
            ContentList[_curDialogueIndex].text = GameTools.Instance.SpeakColorText(_curDialogueList[_curDialogueIndex].text, 3);
            ContentList[_curDialogueIndex].color = new Color(1,1,1);
            // 系统播放原声
            WrongTime = AudicoManager.instance.Play("study", StringConst.DialoguePath + _assetsPath + _curDialogueList[_curDialogueIndex].voice.Replace(".mp3", ""));
            _gameUIInstance.SetVoiceEffect(ListenBtnsList[_curDialogueIndex].gameObject, WrongTime);
        } else
        {
            ContentList[_curDialogueIndex].text = _curDialogueList[_curDialogueIndex].text;
        }
    ...
    {
        ListenBtnsList[_curDialogueIndex].gameObject.SetActive(true);
        ...
        Invoke("PlayDialogueVoice", WrongTime);
    }
    ...
    private void PlayDialogueVoiceEnd()
    {
        ...
        {
            // 扮演A结束，点击下一步扮演B
            UIEventListener.Get(_gameUIInstance.GetNextButton()).onClick = NextRoleB;
        ...
        else
        {
            ...
            {
                // 点击下一步进行下一个话题
                UIEventListener.Get(_gameUIInstance.GetNextButton()).onClick = NextTopic;
                _gameUIInstance.ShowNextButton();
            }
        }
    }
    ...
    /// 扮演下一个角色B
    /// </summary>
    private void NextRoleB(GameObject go)
    {
    ...
    /// </summary>
    /// <param name="go"></param>
    private void NextTopic(GameObject go)
    {
        _gameUIInstance.StopAddDiamond();
        _gameUIInstance.StopVoiceEffect();
        AudicoManager.instance.StopStudyAudio();
